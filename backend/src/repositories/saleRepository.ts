...
export class SaleRepository {
  static async createSale(sale: SaleDTO) {
    ...
    try {
      ...
      const [saleResult]: any = await connection.query(
        `INSERT INTO sales (customer_id, user_id, invoice_number, sale_date, total_amount, tax_amount, discount_amount, paid_amount, payment_status, status, notes)
      ...
      for (const item of sale.items) {
        ...
        await connection.query(
          `INSERT INTO sale_items (sale_id, product_id, variant_id, quantity, unit_price, discount_amount, subtotal)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [
            saleId,
            item.productId,
      ...
      for (const pay of sale.payments) {
        await connection.query(
          `INSERT INTO payments (sale_id, payment_method, amount, transaction_ref, payment_date, notes)
           VALUES (?, ?, ?, ?, NOW(), ?)`,
          [
            saleId,
            pay.paymentMethod,
      ...
      await connection.query(
        `INSERT INTO invoices (sale_id, invoice_number, invoice_date, total_amount, tax_amount, discount_amount, paid_amount, status)
         VALUES (?, ?, NOW(), ?, ?, ?, ?, ?)`,
        [
          saleId,
          invoiceNumber,
  ...
  static async getSaleDetails(saleId: number) {
    ...
    const [items]: any = await pool.query(
      `SELECT si.*, p.name as product_name, pv.variant_name, p.sku as base_sku, pv.sku as variant_sku
       FROM sale_items si
  ...
  static async getInvoice(saleId: number) {
    const [rows]: any = await pool.query(
