...
async function initializeDatabase() {
    ...
    `);
        ...
        // Seed Roles
        const [rolesCheck] = await connection.query('SELECT COUNT(*) as count FROM roles');
      ...
      `);
        ...
        if (usersCheck[0].count === 0) {
            ...
            const [cashierRole] = await connection.query("SELECT id FROM roles WHERE name = 'Cashier'");
            await connection.query(`
      ...
      `, [
        ...
        // Seed Categories
        const [categoriesCheck] = await connection.query('SELECT COUNT(*) as count FROM categories');
        ...
        // Seed Brands
        const [brandsCheck] = await connection.query('SELECT COUNT(*) as count FROM brands');
        ...
        // Seed Suppliers
        const [suppliersCheck] = await connection.query('SELECT COUNT(*) as count FROM suppliers');
      ...
      `);
        ...
        const [productsCheck] = await connection.query('SELECT COUNT(*) as count FROM products');
        if (productsCheck[0].count === 0) {
            // Get category and brand IDs
            const [phoneCat] = await connection.query("SELECT id FROM categories WHERE name = 'Smartphones'");
            const [chargerCat] = await connection.query("SELECT id FROM categories WHERE name = 'Fast Chargers'");
            const [earbudCat] = await connection.query("SELECT id FROM categories WHERE name = 'Wireless Earbuds'");
            const [appleBrand] = await connection.query("SELECT id FROM brands WHERE name = 'Apple'");
            const [samsungBrand] = await connection.query("SELECT id FROM brands WHERE name = 'Samsung'");
            const [ankerBrand] = await connection.query("SELECT id FROM brands WHERE name = 'Anker'");
            // Insert Apple iPhone 15 Pro (Has Variants)
            const [iphoneResult] = await connection.query(`
      ...
      `, [iphoneId, iphoneId]);
            // Seed IMEIs
            const [varNT] = await connection.query("SELECT id FROM product_variants WHERE sku = 'IPH15PRO-NT-128'");
            await connection.query(`
      ...
      `, [iphoneId, varNT[0].id]);
            ...
            const [s24Result] = await connection.query(`
        INSERT INTO products (name, sku, barcode, qr_code, category_id, brand_id, cost_price, retail_price, alert_quantity, status, description, has_variants)
      ...
      `, [chargerId]);
        ...
        const [customersCheck] = await connection.query('SELECT COUNT(*) as count FROM customers');
