...
class ReportRepository {
    static async getDashboardStats() {
        ...
        const [todaySales] = await db_1.default.query(`SELECT COALESCE(SUM(total_amount), 0) as total, COUNT(*) as count 
       FROM sales 
       WHERE DATE(sale_date) = CURDATE() AND status = 'COMPLETED'`);
        const [weeklySales] = await db_1.default.query(`SELECT COALESCE(SUM(total_amount), 0) as total, COUNT(*) as count 
       FROM sales 
       WHERE sale_date >= DATE_SUB(NOW(), INTERVAL 7 DAY) AND status = 'COMPLETED'`);
        const [monthlySales] = await db_1.default.query(`SELECT COALESCE(SUM(total_amount), 0) as total, COUNT(*) as count 
       FROM sales 
       WHERE sale_date >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND status = 'COMPLETED'`);
        const [annualSales] = await db_1.default.query(`SELECT COALESCE(SUM(total_amount), 0) as total, COUNT(*) as count 
       FROM sales 
       WHERE sale_date >= DATE_SUB(NOW(), INTERVAL 365 DAY) AND status = 'COMPLETED'`);
        ...
        const [profit30] = await db_1.default.query(`SELECT 
       ...
       WHERE s.sale_date >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND s.status = 'COMPLETED'`);
        ...
        const [expenses30] = await db_1.default.query(`SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE expense_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)`);
        const expenses = parseFloat(expenses30[0].total);
        const netProfit = grossProfit - expenses;
        // 4. Stock value (Cost value and Retail value)
        const [stockVal] = await db_1.default.query(`SELECT 
       ...
       FROM product_variants`);
        ...
        const [lowStockResult] = await db_1.default.query(`SELECT COUNT(DISTINCT p.id) as count
       FROM products p
       ...
       HAVING SUM(pv.stock_quantity) <= p.alert_quantity`);
        ...
        const [returnsResult] = await db_1.default.query(`SELECT COUNT(*) as count, COALESCE(SUM(total_refund), 0) as total 
       FROM returns 
    ...
    static async getChartsData() {
        ...
        const [monthlyTrends] = await db_1.default.query(`SELECT 
       ...
       ORDER BY DATE_FORMAT(s.sale_date, '%Y-%m') ASC`);
        ...
        const [bestCategories] = await db_1.default.query(`SELECT c.name as category_name, SUM(si.quantity) as units_sold, SUM(si.subtotal) as revenue
       FROM sale_items si
       ...
       LIMIT 5`);
        ...
        const [bestProducts] = await db_1.default.query(`SELECT p.name as product_name, SUM(si.quantity) as units_sold, SUM(si.subtotal) as revenue
       FROM sale_items si
       JOIN products p ON si.product_id = p.id
       JOIN sales s ON si.sale_id = s.id
       WHERE s.status = 'COMPLETED'
       ...
       LIMIT 5`);
        // 4. Top Customers
        const [topCustomers] = await db_1.default.query(`SELECT c.name as customer_name, COUNT(s.id) as visit_count, SUM(s.total_amount) as total_spent
       FROM sales s
    ...
    static async getStockValueReport() {
        const [rows] = await db_1.default.query(`SELECT p.name as product_name, pv.variant_name, pv.sku, pv.stock_quantity, pv.cost_price, pv.retail_price,
    ...
    static async addExpense(expense, userId) {
        const [result] = await db_1.default.query(`INSERT INTO expenses (category, amount, expense_date, notes, user_id)
       VALUES (?, ?, ?, ?, ?)`, [expense.category, expense.amount, expense.expenseDate, expense.notes || '', userId]);
